...
%% 用erlang:port_command来发送数据
port_command_send(Socket, WebSocketBin) ->
    gen_tcp:send(Socket, WebSocketBin),
    %% erlang:port_command(Socket, WebSocketBin, [force, nosuspend]),
    {ok, true}.
protobuf_send(Socket, MsgId, SendMsg) when is_list(Socket) ->
    send_to_niu_sid(Socket, MsgId, SendMsg);
protobuf_send(Socket, MsgId, SendMsg) ->
    protobuf_send(Socket, MsgId, ?E_OK, SendMsg).
protobuf_send(Socket, MsgId, Code, SendMsg) ->
    {ok, BinData} = protobuf:s2c_pack(MsgId, Code, 0, SendMsg),
    lib_send:send_one(Socket, BinData),
    ok.

send_one(Socket, Bin) ->
    case catch port_command_send(Socket, Bin) of
        {ok, true} ->
            next;
        _Reason ->
            %% 关闭socket
            ?ERROR("role ~w send data busy, reason is ~w", [111, _Reason]),
            error
    end.

send_to_niu_sid(PidSend, MsgId, SendMsg) ->
    case PidSend of
        [PidSend1, PidSend2] ->
            case get(rand_to_process) of
                undefined ->
                    put(rand_to_process, 1),
                    PidSend1 ! {send, MsgId, SendMsg};
                _ ->
                    put(rand_to_process, undefined),
                    PidSend2 ! {send, MsgId, SendMsg}
            end;
        _ ->
            skip
    end.
