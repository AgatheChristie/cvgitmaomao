...
login_parse_packet(Socket, Client) ->
    ...
    %%登陆处理
        {inet_async, Socket, Ref, {ok, <<Len:32, Cmd:32>>}} ->
                    ...
                    receive
                        {inet_async, Socket, Ref1, {ok, Binary}} ->
                            case routing(Client, Cmd, Binary) of
                                %%先验证登陆
                                {ok, ?NET_NIU_ROLE_LOGIN_C2S, Data} ->
                                    case pp_account:handle(?NET_NIU_ROLE_LOGIN_C2S, [], Data) of
                                        {true, L, Sn, Accid, Accname} ->
                        ...
                        _ ->
                            case Client#client.login == 1 of
                                true ->
                                    case Cmd =:= 10002 of
                                        true ->
                                            pp_account:handle(Cmd, Socket, [Client#client.accid, Client#client.accname]);
