...
%%     ok = start_delayer(),%%开启延时信息监控树
    ...
    %%开启在线统计监控树		
    start_node_application(start_online_count, [], ?YG_SERVER_NODE_ID_1, Node_id),
    %%开启师徒关系控制树
    start_node_application(start_master_apprentice, [], ?YG_SERVER_NODE_ID_1, Node_id),
    %%开启vip监控树
    start_node_application(start_vip, [], ?YG_SERVER_NODE_ID_1, Node_id),
    %%开启委托监控树
    start_node_application(start_consign, [], ?YG_SERVER_NODE_ID_1, Node_id),
    %% 跨服通信服务端模块
    start_node_application(start_leap_server, [], ?YG_SERVER_NODE_ID_1, Node_id),
    %%递增ID服务
    start_node_application(start_auto_id, [], ?YG_SERVER_NODE_ID_1, Node_id),
    %%开启求购监控树
    start_node_application(start_buy, [], ?YG_SERVER_NODE_ID_1, Node_id),
    %%开启队伍招募服务
    start_node_application(start_team_raise, [], ?YG_SERVER_NODE_ID_1, Node_id),

    %%活动全局进程，分配到独立节点 2

    %%开启氏族监控树	
    start_node_application(start_guild, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %%开启氏族监控树	
    start_node_application(start_title, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %%开启国运监控树
    start_node_application(start_carry, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %%开启排行榜监控树
    start_node_application(start_rank, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %% 开启战场控制树
    ...
    %%开启跨服战场监控树
    start_node_application(start_war_supervisor, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %%开启评价监控树
    start_node_application(start_appraise, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %% 跨服通信客户端模块
    start_node_application(start_leap_client, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %% 开启婚宴监控树
    start_node_application(start_wedding, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %%开启特殊活动进程
    start_node_application(start_event_process, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %%开启斗兽进程
    start_node_application(start_mount_arena, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %%开启全局缓存进程
    start_node_application(start_global_cache, [], ?YG_SERVER_NODE_ID_2, Node_id),
    %%开启跨服单人竞技监控树
    start_node_application(start_war2_server, [], ?YG_SERVER_NODE_ID_2, Node_id),
