...
-define(MYTHROW(ECode),
    case is_integer(ECode) of
        true ->
            ?C2SERR(ECode);
        false ->
            ?C2SERR(#failure_msg_mix{
                string_values = [ECode]
            })
    end
).
...
-define(T(TEXT, PARAMS), util:format(<<(TEXT)/utf8>>, PARAMS)).
...
-define(PRINT(F), io:format("%% [~w~w:~w] " ++ F ++ "~n", [self(), ?MODULE, ?LINE])).
-define(PRINT(F, A), io:format("%% [~w~w:~w] " ++ F ++ "~n", [self(), ?MODULE, ?LINE | A])).
-define(TEST_MSG(Format, Args),
    ?LOG_NOTICE(Format, Args)).
-define(DEBUG(Format),
    ?LOG_DEBUG(Format)).
