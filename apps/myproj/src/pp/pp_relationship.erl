...
%% 请求好友列表
handle(?FRIEND_NIU_INFO_C2S, Player, #friend_niu_info_c2s{} = _Req, _PlayerStatus) ->
...
%% Uid 用户ID
%% Nick 角色名
handle(?FRIEND_NIU_APPLY_C2S, Status, #friend_niu_apply_c2s{type = Type, id = Uid, name = Nick} = _Req, _PlayerStatus) ->
    ?ASSERT(not lib_war:is_war_server(), ?E_UNKONW_DATA),
...
%%Uid:用户ID
handle(?FRIEND_NIU_HUIY_C2S, Status, #friend_niu_huiy_c2s{res = Res, id = Uid} = _Req, _PlayerStatus) ->
    case Res of
        0 ->
            Msg = #friend_niu_huiy_s2c{recer = 2, res = Res, id = Status#player.id, lv = Status#player.lv,
                career = Status#player.career, name = Status#player.nickname},
            lib_send:protobuf_send_to_uid(Uid, ?FRIEND_NIU_HUIY_S2C, Msg);

        1 ->
            %%保证有数据
%%             lib_relationship:set_ets_rela_record(Status#player.id),
            %%查看是否已经加过好友了 todo 这里是不是重复判断了 正常情况下 is_exists 会调用  is_exists_remote
            {_, F1} = lib_relationship:is_exists_remote(Uid, Status#player.id, 1),
            {_, F2} = lib_relationship:is_exists(Status#player.id, Uid, 1),

            ?ASSERT(not (F1 orelse F2), ?E_FRIEND_HAS_BE_FRIEND),
            ...
            %% 对方的好友满了
                His_Friend_Full ->
                    ...
                    %% 给对方发
                    Msg = #friend_niu_huiy_s2c{recer = 2, res = 5},
                    lib_send:protobuf_send_to_uid(Uid, ?FRIEND_NIU_HUIY_S2C, Msg),

                    %% 给自己发
                    lib_relationship:send_friend_niu_huiy_s2c(Status, 1, 5),
            ...
            %% 我的好友满了
                MyFriendNum >= NewMaxFriends ->
                    ...
                    %% 给对方发
                    Msg = #friend_niu_huiy_s2c{recer = 2, res = 4},
                    lib_send:protobuf_send_to_uid(Uid, ?FRIEND_NIU_HUIY_S2C, Msg),

                    %% 给自己发
                    lib_relationship:send_friend_niu_huiy_s2c(Status, 1, 4),
                ...
                true ->
                    ...
                    lib_relationship:send_friend_niu_huiy_s2c(Status, 1, Res, Status#player.id, Status#player.lv,
                        Status#player.career, Status#player.nickname),
                    ...
                    response_friend_request(Data2, Uid, Status#player.id, Id, Close), %%回应好友请求后， 要求请求方做相关处理
                    ok
            end
    end;
    ...
    NewPlayerState =
        case Player#player.lv < 38 of
            ...
            false ->
                ...
                spawn(fun() ->
                    db_agent:insert_log_bottle_exp(NewPlayer#player.id, B_exp, B_spr, TimeStamp, NewPlayer#player.lv) end),
...
pack_friend_list(R, VipList) when is_record(R, ets_rela) ->
    On_off_line =
        case lib_player:is_online(R#ets_rela.rid) of
            true -> 1;
            _ -> 0
        end,
%% 	[VipList] = List, 
...
%%回应好友请求后， 请求方需要做的处理
response_friend_request([A, Res, B, C, _SSSex, D, E], RequestId, ResponseId, Id, PlayerStatus, Close) ->
    ...
    lib_relationship:send_friend_niu_huiy_s2c(PlayerStatus, A, Res, B, C, D, E),
                            ...
                            false ->
                                ...
                                NewState =
                                    if Status1#player.lv < 38 ->
                                        ...
                                        %%(基础数据全额)
                                        spawn(fun() ->
                                            db_agent:set_bless_bottle(Status1#player.id, [NewExp, NewSpr]) end),
                ...
                true ->
                    ...
                    case Player#player.lv < 38 of
                        ...
                        true ->
                            ...
                            %%祝福瓶部分(基础数据全额)
                            spawn(fun() ->
                                db_agent:set_bless_bottle(Player#player.id, [NewExp, NewSpr, NewBlessTimes]) end),
