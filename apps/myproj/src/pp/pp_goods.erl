...
handle(?BAG_NIU_LIST_C2S, PlayerStatus, #bag_niu_list_c2s{type = Location} = _Req) ->
    gen_server:cast(PlayerStatus#player.other#player_other.pid_goods,
        {'list_15010', PlayerStatus, Location}),
    ok;

%% 整理背包
handle(?BAG_NIU_CLEAN_C2S, PlayerStatus, #bag_niu_clean_c2s{} = _Req) ->
    %% 按物品类型ID排序
    ?APPLY_LOCK(3),
    gen_server:cast(PlayerStatus#player.other#player_other.pid_goods,
        {'clean_15052',
...
handle(?BAG_NIU_BUY_C2S, PlayerStatus, #bag_niu_buy_c2s{goods_type_id = GoodsTypeId, goods_num = GoodsNum,
    ...
    Msg = #bag_niu_buy_s2c{res = Res, goods_type_id = GoodsTypeId,
        goods_num = GoodsNum, shop_type = ShopType, new_coin = NewPlayerStatus#player.coin,
        new_bcoin = NewPlayerStatus#player.bcoin, new_cash = NewPlayerStatus#player.cash, new_gold = NewPlayerStatus#player.gold,
        score = NewPlayerStatus#player.arena_score,
        item_list = goods_util:to_p_niu_item_lists(GoodsList)},
    lib_send:protobuf_send(NewPlayerStatus#player.other#player_other.pid_send, ?BAG_NIU_BUY_S2C, Msg),
    {ok, NewPlayerStatus};
