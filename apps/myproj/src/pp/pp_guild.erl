...
%% -----------------------------------------------------------------
%%handle(?GUILD_INFO_C2S, Status, [Realm, Type, Page, GuildName, ChiefName]) ->
handle(?GUILD_LIST_INFO_C2S, Status, #guild_list_info_c2s{realm = Realm, page = Page, type = Type,
    guild_name = GuildName, chief_name = ChiefName} = _Req) ->
    ?APPLY_LOCK(),
    if
        Type =:= 1 orelse (Type =:= 2 andalso Realm > 0 andalso Realm < 4)
            andalso Page >= 1 ->%%只有1，或者2才会做操作
