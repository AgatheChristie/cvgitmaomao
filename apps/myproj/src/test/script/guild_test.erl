...
%            sleep(10),
            % 创建氏族消息40001
%            handle(create_guild, {"潮人帮", "氏族宗旨"}, Socket),
%            sleep(10),
            % 解散氏族消息40002
...
%            sleep(10),
            %% 修改氏族宗旨
%            handle(modify_guild_tenet, {22, "新氏族宗旨"}, Socket),
%            sleep(10),
            %% 修改氏族公告
%            handle(modify_guild_announce, {22, "新氏族公告"}, Socket),
%            sleep(10),
            %% 招纳贤士信息
...
%            sleep(10),
            %%氏族聊天
%            handle(guild_chat, {1, "氏族聊天信息"}, Socket),
%            sleep(10),
            rec(Socket);
        {error, Reason} ->
...
handle(create_guild, {GuildName, GuildIntro}, Socket) ->
     ...
     MsgLen = byte_size( <<1:16,40001:16,NameLen:16,"潮人帮", IntroLen:16, "氏族宗旨">>),
     gen_tcp:send(Socket, <<MsgLen:16,40001:16,NameLen:16,"潮人帮", IntroLen:16, "氏族宗旨">>),
     ok;%rec(Socket);
...
handle(modify_guild_announce, {GuildId, Announce}, Socket) ->
    ...
    MsgLen = byte_size(<<1:16,40016:16,GuildId:32, NameLen:16,"新氏族公告">>),
    gen_tcp:send(Socket, <<MsgLen:16, 40016:16, GuildId:32, NameLen:16, "新氏族公告">>),
    ok;%rec(Socket);
...
handle(guild_chat, {Color, Content}, Socket) ->
    ...
    io:format("client send: guild_chat Cmd=[11005], Color=[~p], Title=[~s] ~n", [Color, "氏族聊天信息"]),
    MsgLen = byte_size(<<1:16,11005:16,Color:8, ContentLen:16, "氏族聊天信息">>),
    gen_tcp:send(Socket, <<MsgLen:16, 11005:16, Color:8, ContentLen:16, "氏族聊天信息">>),
    ok;%rec(Socket);
