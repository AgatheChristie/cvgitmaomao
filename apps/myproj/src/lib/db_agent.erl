        ...
        _ ->%%减少铜币
            KeyValueList =
                case Player#player.bcoin < Coin of
                    ...
                    true ->
                        [{bcoin, 0}, {coin, (Player#player.bcoin + Player#player.coin - Coin)}, {coin_sum, Coin, sub}]
                end,
...
cost_score(PlayerStatus, Cost, Type, PointId) ->
    ...
    case PlayerStatus#player.arena_score > Cost1 of
        true -> ?DB_MODULE:update(player, [{arena_score, Cost1, sub}], [{id, PlayerStatus#player.id}]),
            spawn(fun() ->
                consume_log(PointId, PlayerStatus#player.id, Type, PlayerStatus#player.arena_score, Cost1, 0) end);
    ...
    KeyValueList =
        case Type of
            coin -> case PlayerStatus#player.bcoin < Cost1 of
                        ...
                        true ->
                            [{bcoin, 0}, {coin, (PlayerStatus#player.bcoin + PlayerStatus#player.coin - Cost1)}, {coin_sum, Cost1, sub}]
                    end;
...
rank_get_player_info(POOLID, Realm, Career, Sex, Type, Limit) ->
    ...
    Where1 = [{realm, "<>", ?NEW_PLAYER_SCENE_ID}, {realm, "<>", ?NEW_PLAYER_SCENE_ID_TWO}, {realm, "<>", ?NEW_PLAYER_SCENE_ID_THREE}] ++ lists:filter(fun(T) ->
        is_tuple(T) end, Where0),
...
%%答题模块,随机从题库中抽取Sum条记录
load_base_answer(Sum) -> %% db_mongo:select_count(base_answer, []),
...
get_player_mult_properties(PropertiesList, PlayerIdList) ->
    if PlayerIdList == [] orelse PropertiesList == [] ->
        ...
        true ->
            Propertiestring =
                case lists:member(id, PropertiesList) of
                    true ->
                        util:list_to_string(PropertiesList);
                    false ->
                        util:list_to_string(PropertiesList ++ [id])
                end,
