...
get_guild_page(PidSend, _OwnGId, Realm, Type, Page, GuildName, ChiefName) ->
    ?DEBUG("OwnGId:~p, Realm:~p, Type:~p, Page:~p, GuildName:~p, ChiefName:~p", [_OwnGId, Realm, Type, Page, GuildName, ChiefName]),
    Pattern =
        case Type of
            1 ->
                case Realm of
...
%%	TPage = util:ceil(Total/Limit),
    %%求需要返回的分页
    Guilds = ets:match_object(?ETS_GUILD, Pattern),
%%	Guilds =
%%		case Page > TPage of
%%			true ->%%发的分页太大了，没数据啦
...
%%	?DEBUG("the result guilds len is ~p, ~p, ~p", [Total, NewPage, TPage]),
    Msg = #guild_list_info_s2c{total = Page, new_page = Page, guild_infos = to_p_guild_list_infos(Guilds)},
    lib_send:protobuf_send(PidSend, ?GUILD_LIST_INFO_S2C, Msg),
    ok.
%%	{ok, BinData} = pt_40:write(40010, [TPage, NewPage, Records]),
%%    lib_send:send_to_sid(PidSend, BinData).

to_p_guild_list_infos(Infos) ->
    [to_p_guild_list_info(X) || X <- Infos].
...
handle_guild_page(#ets_guild{id = GuildId, name = GuildName, chief_id = ChiefId, chief_name = ChiefName,
