...
%% 	Status = lib_vip:check_vip_state(PlayerStatus),
    ...
    Msg = #role_niu_info_s2c{
        ...
        nick_name = Status#player.nickname,
        max_att = Status#player.max_attack,
        min_att = Status#player.min_attack,
        def = Status#player.def,
        forza = Status#player.forza,
        physique = Status#player.physique,
        agile = Status#player.agile,
        wit = Status#player.wit,
        hit = Status#player.hit,
        dodge = Status#player.dodge,
        crit = Status#player.crit,
        guild_id = Status#player.guild_id,
        guild_name = Status#player.guild_name,
        realm = Status#player.realm,
        gold = Status#player.gold,
        cash = Status#player.cash,
        ...
        e4 = E4,
        e5 = E5,
        mount = Status#player.mount,
        pk_mode = Status#player.pk_mode,
        %%title = Status#player.title,
        couple_name = Status#player.couple_name,
        %%position = Status#player.position,
        evil = Status#player.evil,
        realm_honor = Status#player.realm_honor,
        culture = Status#player.culture,
        state = Status#player.state,
        ...
        stren = Status#player.other#player_other.stren,
        suit_id = Status#player.other#player_other.suitid,
        change_reason = ChangeReason,
        arena = Status#player.arena_score,
        vip = Status#player.vip,
        vip_time = Status#player.vip_time,
        mount_stren = Status#player.other#player_other.mount_stren,
        is_spring = Status#player.other#player_other.is_spring,
        ach_hp = AchHp, ach_mp = AchMp, ach_att = AchAtt, ach_def = AchDef, ach_dod = AchDod, ach_hit = AchHit, ach_crit = AchCrit, ach_anit = AchAnti,
    ...
    lib_send:protobuf_send(Status#player.other#player_other.pid_send, ?ROLE_NIU_INFO_S2C, Msg),
    ok.
                                            ...
                                            false ->
                                                ...
                                                case lib_coliseum:is_coliseum_scene(SceneId) of
                                                    false ->
                                                        %% 普通场景死亡
                                                        spawn(fun() ->
                                                            normal_die(Player, AerPid, AerId, Realm, NickName, Career) end),
                                                        %% 红名惩罚
...
%% 									   %%结了婚不可以变性
                                       if Player#player.sex =:= 1 ->
                                           Ms = ets:fun2ms(fun(M) when M#ets_marry.boy_id =:= Player#player.id ->
                                               M end);
                                           true ->
                                               Ms = ets:fun2ms(fun(M) when M#ets_marry.girl_id =:= Player#player.id ->
                                                   M end)
                                       end,
                            ...
                            [] ->
                                ...
                                case goods_util:get_ets_list(?ETS_GOODS_ONLINE, Pattern4) of
                                    ...
                                    [Info4] ->
                                        ...
                                        spawn(fun() ->
                                            db_agent:mid_fashion_change(Info4#goods.id, NewTypeId4, NewIcon) end),
                ...
                fun(FaBaoElem) ->
                    case get_fashion_fb_sp_change(FaBaoElem#goods.goods_id) of
                        ...
                        NFaBaoGoodsTypeId ->
                            ...
                            spawn(fun() ->
                                db_agent:mid_fashion_change(FaBaoElem#goods.id, NFaBaoGoodsTypeId, FBNIcon) end)
                    end
                end,
