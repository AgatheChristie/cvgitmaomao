...
to_p_friend_niu_infos(Infos) ->
    [to_p_friend_niu_info(X) || X <- Infos].

to_p_friend_niu_info([Online, IdB, Lv, Sex, Career, Nick, Id, Close, Vip]) ->
    #p_friend_niu_info{online = Online, id_b = IdB, lv = Lv, sex = Sex, career = Career,
        nick = Nick, id = Id, close = Close, vip = Vip}.
...
find_is_exists(IdA, IdB, Rela) ->
    Ms = ets:fun2ms(fun(R) when R#ets_rela.pid == IdA andalso R#ets_rela.rid == IdB andalso R#ets_rela.rela == Rela ->
        R#ets_rela.id end),
    Ms1 = ets:fun2ms(fun(R) when R#ets_rela.pid == IdB andalso R#ets_rela.rid == IdA andalso R#ets_rela.rela == Rela ->
        R#ets_rela.id end),
    ...
    case lib_player:get_player_pid(Remote_id) of
        ...
        Pid ->
            case catch gen:call(Pid, '$gen_call', {count_fri}, 2000) of
                %% todo 这里是有问题的  返回值匹配不对 会一直走到 下面 _ 返回true 如果玩家在线的话
                %% @doc 看了一下源码 没啥问题 gen_server会把ok ret里面的ok去掉
                {ok, Ret} ->
                    Ret;
                _E ->
                    ?DEBUG("_E:~p end", [_E]),
...
get_friend_list(Player) ->
    ...
    Msg = #friend_niu_info_s2c{friend_infos = lib_relationship:to_p_friend_niu_infos(L2)},
    lib_send:protobuf_send(Player#player.other#player_other.pid_send, ?FRIEND_NIU_INFO_S2C, Msg),
    ok.
...
check_send_friend_request(Status, Uid, Nick) ->
    ?ASSERT(is_integer(Uid), ?E_NOT_THIS_VIA),
    IdB =
        case Uid of
            0 ->
                ?ASSERT(not (Nick =:= Status#player.nickname), ?E_FRIEND_CAN_NOT_ADD_MYSELF),
                Tmp = lib_player:get_role_id_by_name(Nick),
                ?DEBUG("Tmp:~p end", [Tmp]),
                ?ASSERT(Tmp =/= null, ?E_FRIEND_CAN_NOT_ADD_MYSELF),
                Tmp;
            _ ->
                ?ASSERT(not (Uid =:= Status#player.id), ?E_FRIEND_CAN_NOT_ADD_MYSELF),
                Uid
        end,
    case lib_player:is_online(IdB) of
        ...
        true ->
            %%检查是否已经存在好友关系
            case ets:match(?ETS_RELA, #ets_rela{id = '$1', pid = Status#player.id, rid = IdB, rela = 1, _ = '_'}) of
                [] ->
                    {online, IdB};
                [[{_, _, _}] | _T] ->
                    throw(?E_FRIEND_HAS_BE_FRIEND)
            end
...
send_friend_request(Status, Type, Uid, Nick) ->
    Val = check_send_friend_request(Status, Uid, Nick),
    case Val of
        %%回应方在线
        {online, Id} ->
            %%保证有数据
            case is_exists_remote(Id, Status#player.id, 2) of    %%自己是否在对方的黑名单中
                {offline, _} ->
                    %% 回应方不在线
                    send_friend_niu_huiy_s2c(Status, 2, 2);
                {ok, false} ->
...
%%                            {ok, BinData} = pt_14:write(14001, Data1),
                            Msg = #friend_niu_apply_s2c{type = Type, id = Status#player.id,
                                lv = Status#player.lv, career = Status#player.career, name = Status#player.nickname},
                            lib_send:protobuf_send_to_uid(Id, ?FRIEND_NIU_APPLY_S2C, Msg),
                            lib_chat:send_sys_msg_one(Status#player.other#player_other.socket, ?T("加好友请求已发出，等待对方回应"));
                        {_Rid, true} ->
                            %%回应方已经加了发送方为好友 
                            Rela_ets = lib_relationship:add(Status#player.id, Id, 1, 0),
...
%%                                Rela_ets#ets_rela.career, Rela_ets#ets_rela.nickname],
                            send_friend_niu_huiy_s2c(Status, 2, 1, Rela_ets#ets_rela.rid, Rela_ets#ets_rela.lv,
                                Rela_ets#ets_rela.career, Rela_ets#ets_rela.nickname)
...
%%                            lib_send:send_to_sid(Status#player.other#player_other.pid_send, BinData)
                ...
                %% 在对方黑名单中
                {_, true} ->
                    send_friend_niu_huiy_s2c(Status, 2, 3)
            end;
        %% 回应方不在线
        {offline, _} ->
            send_friend_niu_huiy_s2c(Status, 2, 2)
...
send_friend_niu_huiy_s2c(Status, Recer, Res) ->
    send_friend_niu_huiy_s2c(Status, Recer, Res, 0, 0, 0, []).
send_friend_niu_huiy_s2c(Status, Recer, Res, Id, Lv, Career, Name) ->
    Msg = #friend_niu_huiy_s2c{recer = Recer, res = Res, id = Id, lv = Lv, career = Career, name = Name},
    lib_send:protobuf_send(Status#player.other#player_other.pid_send, ?FRIEND_NIU_HUIY_S2C, Msg).
...
insert_to_db(PlayerId) ->
    ...
    F = fun(R) ->
        ...
        if R#ets_rela.close > 1000 ->
            spawn(fun() ->
                db_agent:log_close_add(DbId, R#ets_rela.pid, R#ets_rela.rid, R#ets_rela.close, util:unixtime()) end);
            true ->
                skip
        end
        end,
    ...
    case lib_player:get_player_pid(IdA) of
        ...
        Apid -> case lib_player:get_player_pid(IdB) of
                    ...
                    Bpid ->
                        ...
                        %%送花，即时写亲密度日志
                        spawn(fun() ->
                            db_agent:log_close_add(DbId, IdA, IdB, NewRela#ets_rela.close, util:unixtime()) end)
                end
    end;
...
del_close(IdA, IdB, Close, Type) ->
    case ets:match_object(?ETS_RELA, #ets_rela{pid = IdA, rid = IdB, rela = 1, _ = '_'}) of
        ...
        [Rela] ->
            ...
            spawn(fun() ->
                db_agent:update_close(Id, NewRela#ets_rela.close, NewRela#ets_rela.pk_mon, NewRela#ets_rela.timestamp) end),
