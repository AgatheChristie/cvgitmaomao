...
%% 检查行会名字合法性
check_hanghui_name(HanghuiName) ->
    %% 过滤非法字符
    NewHanghuiName = ?L2B(HanghuiName),
    ?ASSERT(util:name_valid(NewHanghuiName), ?T("名字不合法")),
    ?ASSERT(util:is_text_valid(HanghuiName, 4, 12), ?T("名字编码不合法")).
...
%% 40001 创建氏族
%% -----------------------------------------------------------------
create_guild(MyGuildId, _Level, Coin, BCoin, _GoodsPid, PlayerId, PlayerName, Realm, Pid,
        [GuildName, UserItem]) ->
    ?DEBUG("GuildName:~p end", [GuildName]),
    %%判断名字长度是否合法（2个字符到10个字符之间， 一个中文当两个字符算,而且不能存在非法字符）
    %%获取创建氏族的所需铜币
    #{create_coin := CreateCoin} = ?ASSERT_MAP(base_guild:get(1), ?E_SYS_CONFIGH_ERR),
    check_hanghui_name(GuildName),
    %% 新手不能创建氏族
    ?ASSERT(Realm =/= 100, ?T("还是新手")),
    %%你拥有了氏族
    ?ASSERT(MyGuildId =:= 0, ?T("你已经拥有氏族了")),
%%    NewGuildName = tool:to_binary(GuildName),
    %% 重名
    ?ASSERT(lib_guild_inner:get_guild_by_name(GuildName) =:= [], ?T("氏族名字重复")),
    %% 等级不够
%%    ?ASSERT(Level >= ?CREATE_GUILD_LV, ?T("创建氏族等级不足")),
    case UserItem of
        0 -> %%使用建帮令
            ?MYTHROW(?T("功能未开启"));
%%            NewGuildName = tool:to_binary(GuildName),
...
%%            end;
        1 -> %%使用金币建帮
            if    %%判定铜币是否足够:（铜+绑定铜一起计算）。
                (Coin < 0) orelse (BCoin < 0) orelse (CreateCoin < 0) orelse (Coin + BCoin) < CreateCoin ->
                    ?MYTHROW(?T("铜币不足"));
                true ->
                    case lib_guild_inner:create_guild(PlayerId, PlayerName, Realm, GuildName, 0, 0) of
                        {ok, GuildId} ->
                            gen_server:cast(Pid, {'guild', null, PlayerId}),
                            {ok, [1, GuildId]};
                        _ ->
...
apply_join_guild(My_id, My_nickname, My_guild_id, My_realm, My_lv, My_quit_guild_time, Pid, PlayerSex, PlayerJobs, PlayerCareer, PlayerVip, [GuildId]) ->
    ...
    Guild = lib_guild_inner:get_guild(GuildId),
    ?ASSERT(Guild =/= [], ?E_ROLE_RELOAD_DATA_TIMES),
    gen_server:cast(Pid, {'guild', null, My_id}),
    #ets_guild{realm = GuildRealm, member_num = GuildMemberNum} = Guild,
    %%计算氏族人数上限
    PopulationLevel = lib_guild:get_guild_skill_level(GuildId, 3),
    #{guild_member_base := CCC} = ?ASSERT_MAP(base_guild:get(1), ?E_SYS_CONFIGH_ERR),
    GuildMemberCapacity = CCC + PopulationLevel * 5,
    TimeNow = util:unixtime(),
    TimeRest = TimeNow - My_quit_guild_time,
    ApplyNum = lib_guild_inner:get_guild_apply_num(GuildId),
    %%还是新手
    ?ASSERT(My_realm =/= 100, ?T("还是新手")),
    ?ASSERT(?GUILD_APPLY_NUM_LIMIT > ApplyNum, ?T("申请队列上限")),
    %% 您已经有工会了
    ?ASSERT(My_guild_id =:= 0, ?T("您已经有工会了")),
    %% 不是一个国家的
    ?ASSERT(My_realm =:= GuildRealm, ?T("不是一个国家的")),
    %% 人数上限
    ?ASSERT(GuildMemberCapacity > GuildMemberNum, ?T("人数上限")),
    %% 等级不够
%%    ?ASSERT(?CREATE_GUILD_LV >= My_lv, ?T("等级不够")),
    %% 入盟冷却
    ?ASSERT(TimeRest > ?QUIT_GUILD_TIME_LIMIT, ?T("入盟冷却")),
    GuildApply = lib_guild_inner:get_guild_apply_by_player_id(My_id, GuildId),
    %% 已经申请本工会了
    ?ASSERT(GuildApply =:= [], ?T("已经申请本工会了")),

    lib_guild_inner:add_guild_apply(My_id, My_nickname, My_lv, PlayerSex, PlayerJobs, PlayerCareer, PlayerVip, GuildId).


%%计算氏族人数上限
calc_member_max_num(GuildId) ->
    PopulationLevel = lib_guild:get_guild_skill_level(GuildId, 3),
    #{guild_member_base := Base} = ?ASSERT_MAP(base_guild:get(1), ?E_SYS_CONFIGH_ERR),
    Base + PopulationLevel * 5.
...
approve_guild_apply(OfficeGuildId, _OfficeGuildName, OfficeGuildPosition, [HandleResult, ApplyList]) ->
    ...
    ?ASSERT(ApplyListLen > 0, ?T("申请列表为空")),
    ?ASSERT(OfficeGuildId > 0, ?T("您的行会id有问题")),
    ?ASSERT(OfficeGuildPosition =< 7, ?T("您的权限不足")),
    Guild = lib_guild_inner:get_guild(OfficeGuildId),
    ?ASSERT(is_record(Guild, ets_guild), ?T("没有此行会")),
    #ets_guild{realm = GuildRealm, name = GuildName, member_num = MemberNum} = Guild,
    %%计算氏族人数上限
    MemberCapacity = calc_member_max_num(OfficeGuildId),
...
kickout_guild(PlayerId, MyPlayerName, GuildId, My_guild_position, My_guild_depart_id, [KickOutPlayerId]) ->
    ?ASSERT(GuildId =/= 0, ?T("你没有加入任何氏族")),
    ?ASSERT(My_guild_position =< 7, ?T("你没有踢出权限")),
    ?ASSERT(KickOutPlayerId =/= PlayerId, ?T("不能踢出自己")),

    PlayerInfo = lib_guild_inner:get_player_guild_info(KickOutPlayerId),
    Guild = lib_guild_inner:get_guild(GuildId),

    ?ASSERT(PlayerInfo =/= [], ?T("对方玩家不存在")),
    ?ASSERT(Guild =/= [], ?T("氏族数据缺失")),


    [PlayerNickname, _PlayerRealm, PlayerGuildId, _PlayerGuildName,
        PlayerGuildPosition, _PlayerLv, _PlayerQuitGuildTime,
        _PlayerSex, _PlayerJobs, _PlayerLastLoginTime, _PlayerOnlineFlag,
        _PlayerCareer, _PlayerCulture, _PlayerDepartMentId, _PlayerVip] = PlayerInfo,
    ...
    ?ASSERT(PlayerGuildId =/= 0, ?T("对方没有氏族")),
    ?ASSERT(PlayerGuildId =:= GuildId, ?T("对方不是本帮成员")),
    ?ASSERT(PlayerGuildPosition > My_guild_position, ?T("对方职位不在你之下")),
...
%%            [9, <<>>];
    ...
    case lib_guild_inner:remove_guild_member(0, KickOutPlayerId, GuildId, DeputyChiefId1, DeputyChiefId2) of
        [ok, MemberType] ->
            %%计算氏族人数上限
%%            PopulationLevel = lib_guild:get_guild_skill_level(GuildId, 3),
%%            MemberCapacity = data_guild:get_guild_config(guild_member_base, []) + PopulationLevel * 5,
            MemberCapacity = calc_member_max_num(GuildId),
            % 更新缓存
            GuildNew =
                case MemberType of
                    0 ->
                        TGuildNew = Guild#ets_guild{member_num = MemberNum - 1},
                        %%氏族联盟数据更新处理
                        lib_guild_union:update_guild_info(upmem, {TGuildNew#ets_guild.id, TGuildNew#ets_guild.member_num, MemberCapacity}),
                        TGuildNew;
                    1 ->
                        TGuildNew = Guild#ets_guild{member_num = MemberNum - 1,
...
%%            erlang:spawn(lib_guild_inner, kickout_guild_log, [Guild, MyPlayerName, PlayerNickname]),
            lib_guild_inner:update_guild(GuildNew),
            Content = util:format(?T("~s表现不佳，被~s开除出氏族！"), [PlayerNickname, MyPlayerName]),
%%            lib_guild_inner:jion_guild_logqqq(GuildId, Guild#ets_guild.name, Content),
            erlang:spawn(lib_guild_inner, jion_guild_logqqq, [GuildId, Guild#ets_guild.name, Content]),
            {ok, [1, PlayerNickname]};
        _ ->
            ?MYTHROW(?T("踢出成员出错！"))
    end.
...
quit_guild(PlayerId, PlayerNickName, GuildId, GuildPosition, [QuitTime]) ->
    ...
    ?ASSERT(Guild =/= 0, ?T("你尚未加入任何氏族")),

    ?ASSERT(GuildPosition >= 12, ?T("你拥有职位不能直接退出")),


    case lib_guild_inner:remove_guild_member(QuitTime, PlayerId, GuildId, DeputyChiefId1, DeputyChiefId2) of
        [ok, MemberType] ->
            %%计算氏族人数上限
%%            PopulationLevel = lib_guild:get_guild_skill_level(GuildId, 3),
%%            MemberCapacity = data_guild:get_guild_config(guild_member_base, []) + PopulationLevel * 5,
            ...
            lib_guild_inner:update_guild(GuildNew),
            Content = util:format(?T("~s遗憾地退出了氏族！"), [PlayerNickName]),
            lib_guild_inner:jion_guild_logqqq(GuildId, Guild#ets_guild.name, Content),
            {ok, ?TRUE};
        _ ->
            ?MYTHROW(?T("退出氏族失败"))
...
modify_guild_announce(My_guild_id, My_guild_position, [GuildId, Announce]) ->
    ...
    ?ASSERT(Guild =/= 0, ?T("你尚未加入任何氏族")),
    ?ASSERT(Guild =:= My_guild_id, ?T("你不是该氏族成员")),

    ?ASSERT(My_guild_position =< 2, ?T("你无权修改")),

    ?ASSERT(util:is_text_valid(Announce, 0, 100), ?T("公会公告长度不正确")),

    ?ASSERT(util:check_text(Announce), ?T("行会公告有非法字符")),

    case lib_guild_inner:modify_guild_announce(GuildId, Announce) of
        ok ->
            % 更新缓存
%%            AnnounceBin = tool:to_binary(Announce),
            GuildNew = Guild#ets_guild{announce = Announce},
            lib_guild_inner:update_guild(GuildNew),
            {ok, [1]};
        _ ->
            ?MYTHROW(?T("修改公共失败"))
...
%%            lib_send:send_to_sid(Status#player.other#player_other.pid_send, Bin),
            Msg = #guild_notice_join_s2c{player_id = PlayerId, player_name = PlayerName, guild_id = GuildId, guild_name = GuildName},
            lib_send:protobuf_send(Status#player.other#player_other.pid_send, ?GUILD_NOTICE_JOIN_S2C, Msg),
            %%做加入氏族的成就统计
...
%%            end,
            Status1;
        false ->% 其他人加入 todo 目前没有看到能走到这里来的情况
            ?DEBUG("awdafawffffffff end"),
            % 发送通知
%%            {ok, Bin} = pt_40:write(40000, [1, PlayerId, PlayerName, GuildId, GuildName]),
%%            lib_send:send_to_sid(Status#player.other#player_other.pid_send, Bin),
            Status
...
%%            lib_send:send_to_sid(Status#player.other#player_other.pid_send, Bin),
            Msg = #guild_notice_kickout_s2c{player_id = PlayerId, player_name = PlayerName, guild_id = GuildId, guild_name = GuildName},
            lib_send:protobuf_send(Status#player.other#player_other.pid_send, ?GUILD_NOTICE_KICKOUT_S2C, Msg),
...
cast_guild_reject_invite(Param, Status) ->
    ...
    lib_send:send_to_sid(Status#player.other#player_other.pid_send, Bin),
    Status.
cast_guild_reject_apply([GuildId, GuildName], Status) ->
    Msg = #guild_notice_reject_s2c{guild_id = GuildId, guild_name = GuildName},
    lib_send:protobuf_send(Status#player.other#player_other.pid_send, ?GUILD_NOTICE_REJECT_S2C, Msg),
                        ...
                        true ->%%超过时间限制了，可以弹劾
                            case lib_guild_inner:demise_chief(ChiefId, ChiefName, PlayerId, PlayerName,
                                ...
                                [ok, MemberType] ->
                                    ...
                                    %%处理氏族弹劾后的操作
                                    spawn(fun() ->
                                        handle_accuse_info(PlayerId, PlayerName, ChiefId, ChiefName, GuildId, GuildName) end),
                                    {ok, 1};
                                _ ->
                                    {ok, 0}
                            end;
