...
add_goods(GoodsInfo, AttributeList) ->
    NewGoodsInfo =
        case db_agent:add_goods(GoodsInfo) of
            {mongo, Ret} ->
                GoodsInfo#goods{id = Ret};
            _Ret ->
                goods_util:get_add_goods(GoodsInfo#goods.player_id,
...
add_goods_base(GoodsStatus, GoodsTypeInfo, GoodsNum, GoodsInfo, GoodsList) ->
    ...
    case GoodsTypeInfo#ets_base_goods.max_overlap > 1 of
        ...
        false ->
            ...
            [NewGoodsStatus, _, _] = lists:foldl(fun add_nonlap_goods/2,
                [GoodsStatus, GoodsInfo, 4], AllNums)
    end,
    {ok, NewGoodsStatus}.
...
%% 添加新的不可叠加物品   不检查空间  玩家id  GoodsStatus在下面
...
delete_all_box_goods_attribute(GoodsList) ->
    lists:foreach(fun(Goods) ->
        spawn(fun() -> db_agent:delete_all_box_goods_attribute(Goods#goods.id) end) end, GoodsList).
...
get_on_mount(PlayerStatus, MountInfo) ->
    if
        is_record(MountInfo, goods) ->
            ...
            spawn(fun() ->
                mod_scene_agent:send_to_area_scene(NewPlayerStatus#player.scene, NewPlayerStatus#player.x, NewPlayerStatus#player.y, BinData) end),
            NewPlayerStatus;
        true ->
            PlayerStatus
    end.
...
is_enough_backpack_cell(GoodsStatus, GoodsTypeInfo, GoodsNum) ->
    ...
    %% 正常情况下来说这列表里面只有一个值  多个也有可能 比如  位置1 绑定桃子 位置2 长弓 位置3 绑定桃子
    GoodsList1 = [Good || Good <- GoodsList, Good#goods.cell < 200],  %% 大于200就是仓库了
