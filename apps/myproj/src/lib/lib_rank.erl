...
loop_query_batt_value(Limit) ->
    ...
    %% ?DEBUG("PlayerValueList:~p end", [PlayerValueList]),
    PlayerIdList = [_Player_Id || [_Player_Id, _BattValue] <- BattRanks1],
    %% ?DEBUG("PlayerIdList:~p end", [PlayerIdList]),
    PlayerInfoList = db_agent:get_player_mult_properties([id, nickname, sex, career, realm, guild_name, vip], lists:flatten(PlayerIdList)),
    %% ?DEBUG("PlayerInfoList:~p end", [PlayerInfoList]),
    F = fun([Id, Nickname, Sex, Career, Realm, Guild_name, Vip]) ->
        {_Id, Value} =
            case lists:keyfind(Id, 1, PlayerValueList) of
                {A,C} ->
                    {A,C};
                false ->
                    ?DEBUG("Id:~p end", [Id]),
...
get_role_rank1(Realm, Career, Sex, TypeNum) ->
    ...
    case Data of
        ...
        _ ->
            Data1 =
                if Realm > 0 ->
                    lists:filter(fun([_Id1, _Nickname1, _Sex1, _Career1, Realm1, _Guild_name1, _TypeValue1, _Vip]) ->
                        Realm1 == Realm end, Data);
                    true ->
                        Data
                end,
            Data2 =
                if Career > 0 ->
                    lists:filter(fun([_Id2, _Nickname2, _Sex2, Career2, _Realm2, _Guild_name2, _TypeValue2, _Vip]) ->
                        Career2 == Career end, Data1);
                    true ->
                        Data1
                end,
            Data3 =
                if Sex > 0 ->
                    lists:filter(fun([_Id3, _Nickname3, Sex3, _Career3, _Realm3, _Guild_name3, _TypeValue3, _Vip]) ->
                        Sex3 == Sex end, Data2);
                    true ->
                        Data2
                end,
...
award_single_td_get(PlayerStatus) ->
    Res = case select_single_td_award(PlayerStatus#player.id) of
              ...
              [Award] ->
                  ...
                  spawn(fun() ->
                      db_agent:log_single_td_award(PlayerStatus#player.id, Award#ets_single_td_award.order, Award#ets_single_td_award.lv, Spt, BCoin, util:unixtime()) end),
                  1
          end,
