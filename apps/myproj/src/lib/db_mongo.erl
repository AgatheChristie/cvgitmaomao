    ...
    case ResuList of
        ...
        _ ->
            ...
            case Size of
                ...
                1 ->
                    [Value] = [proplists:get_value(list_to_binary(tool:to_list(lists:nth(1, Fields_sql_List))), ResuList)],
...
%%		?DEBUG("SELECT_ALL:~p / ~p  /~p ~n",[Table_name,Fields_sql_string,Where_List]),
    [WhereOpertion, FieldOpertion] = db_mongoutil:make_select_opertion(transfer_fields(Table_name, Fields_sql_string), Where_List, Orderby_sql_List, Limit_sql_List),
    %%?DEBUG("WhereOpertion:~p end", [WhereOpertion]),
	%% ?DEBUG("FieldOpertion:~p end", [FieldOpertion]),
...
handle_all_result(_Table_name, _Fields_sql_List, ResuList = [[{<<"$err">>, _}, _, _]]) ->
    ?ERROR("Err ResuList:~p end", [ResuList]);
