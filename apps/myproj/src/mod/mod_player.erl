...
handle_cast({'SOCKET_EVENT', Cmd, Bin}, PlayerState) ->
    ...
    try routing(Cmd, Player, Bin, PlayerState) of
        %% 修改ets和status
        {ok, NewPlayer} ->
            save_online_diff(Player, NewPlayer),
            NewPlayerState = PlayerState#player_state{
        ...
        T:E:S ->
            ?ERROR("[~p]  handle cmd ~p msg ~p return error, ~p stacktrace ~p,~n",
                [Player#player.id, Cmd, Bin, {T, E}, S]),
            {noreply, PlayerState}
...
handle_cast({'UPDATE_ACITVITY_DATA', AtomType, Param}, PlayerState) ->
    Player = PlayerState#player_state.player,
    spawn(fun() ->
        lib_activity:update_activity_data(AtomType, Player#player.other#player_other.pid, Player#player.id, Param) end),
    {noreply, PlayerState};
...
handle_info({'MASTER_LEAD'}, PlayerState) ->
    Player = PlayerState#player_state.player,
    if
        Player#player.lv < 10 orelse Player#player.lv > 36 ->
            skip;
...
handle_info({'end_do_marry', RName, DbId}, PlayerState) ->
    ...
    erlang:spawn(fun() ->
        catch (lib_achieve:check_achieve_finish_cast(Player#player.other#player_other.pid, 632, [1])) end),%%婚宴成就判断
...
unload_player_info(PlayerState) ->
    ...
    case Pretimes of
        ...
        _ ->
            spawn(fun() ->
                db_agent:set_bless_info(NewPlayer#player.id, Pretimes, PlayerState#player_state.last_bless_time) end)
    end,
