        ...
        case is_record(GoodsInfo, goods) of
            true when GoodsInfo#goods.player_id == GoodsStatus#goods_status.player_id ->
                case goods_util:has_attribute(GoodsInfo) of
                    true ->
                        AttributeList = goods_util:get_goods_attribute_list(GoodsStatus#goods_status.player_id, GoodsId);
                    false -> AttributeList = []
                end,
...
handle_cast({'list_15010', PlayerStatus, Location}, GoodsStatus) ->
    ...
    Msg = #bag_niu_list_s2c{location = NewLocation_1,
        cell_num = CellNum_1, coin = PlayerStatus#player.coin, bcoin = PlayerStatus#player.bcoin,
        cash = PlayerStatus#player.cash,
        gold = PlayerStatus#player.gold, can_sell_bcoin_num = BcoinNum - BcoinTotalNum,
        can_sell_coin_num = CoinNum - CoinTotalNum, item_list = goods_util:to_p_niu_item_lists(GoodsList_1)},
    lib_send:protobuf_send(PlayerStatus#player.other#player_other.pid_send, ?BAG_NIU_LIST_S2C, Msg),
...
handle_cast({'shop_15013', ShopType, ShopSubtype}, GoodsStatus) ->
    ...
    Msg = #bag_niu_shop_s2c{shop_type = ShopType, shop_sub_type = ShopSubtype, result = 1,
        shop_list = goods_util:qqcwfgg(GoodsStatus#goods_status.player_id, ShopType, ShopSubtype, ShopList)},
    lib_send:protobuf_send(GoodsStatus#goods_status.pid_send, ?BAG_NIU_SHOP_S2C, Msg),
                ...
                F = fun(GoodsInfo) ->
                    ...
                    case is_record(GoodsInfo, goods) of
                        true when GoodsInfo#goods.player_id == GoodsStatus#goods_status.player_id ->
                            case goods_util:has_attribute(GoodsInfo) of
                                true ->
                                    AttributeList = goods_util:get_goods_attribute_list(GoodsStatus#goods_status.player_id, GoodsInfo#goods.id);
                                false -> AttributeList = []
                            end,
                            SuitNum = goods_util:get_suit_num(GoodsStatus#goods_status.equip_suit, GoodsInfo#goods.suit_id),
                            %%加成计算
            ...
            {ok, GoodsInfo} ->
                ...
                case (catch lib_goods:delete_one(GoodsStatus, GoodsInfo, GoodsNum)) of
                    {ok, NewStatus, _} ->
                        ...
                        spawn(fun() ->
                            log:log_throw([PlayerStatus#player.id, PlayerStatus#player.nickname, GoodsInfo#goods.id, GoodsInfo#goods.goods_id,
...
handle_cast({'clean_15052', Cell_num}, GoodsStatus) ->
    ...
    Msg = #bag_niu_clean_s2c{item_list = goods_util:to_p_niu_item_lists(NewGoodsList)},
    lib_send:protobuf_send(GoodsStatus#goods_status.pid_send, ?BAG_NIU_CLEAN_S2C, Msg),
                    ...
                    {ok, _, NewPlayer, NewStatus, NewNum, _Buff} ->
                        ...
                        case FestivalType of
                            ...
                            2 ->%%情人节玫瑰
                                ...
                                %%成就系统统计接口
                                erlang:spawn(fun() ->
                                    catch (lib_achieve:check_achieve_finish_cast(Player#player.id, 609, [FlowerNum])) end),
                                %%氏族祝福任务判断
                                GWParam = {1, 1},
                                ...
                                if
                                    Player#player.nickname /= SNickname ->
                                        ...
                                        %%增加好友亲密度
                                        spawn(fun() ->
                                            lib_relationship:close(flower, Player#player.id, To_id, [FlowerNum, Player#player.other#player_other.pid_team, TaTeamId]) end),
                                        %%都需要弹窗
                                        ...
                                        spawn(fun() ->
                                            db_agent:log_charm([Player#player.id, 1, Player#player.id, FlowerNum, FlowerNum, NowTime]) end),
                                        gen_server:cast(To_pid, {'add_charm', FlowerNum}),
                                        spawn(fun() ->
                                            db_agent:log_charm([To_id, 1, Player#player.id, FlowerNum, FlowerNum, NowTime]) end),
                                        %%增加经验灵力
...
handle_call({'pay', PlayerStatus, GoodsTypeId, GoodsNum, ShopType, ShopSubtype}, _From, GoodsStatus) ->
    case catch check_pay(PlayerStatus, GoodsStatus, GoodsTypeId, GoodsNum, ShopType, ShopSubtype) of
        {fail, Res} ->
                    ...
                    case Result of
                        1 ->
                            ...
                            case lists:member(GoodsInfo#goods.goods_id, [23409, 23410, 23411]) of
                                true -> spawn(fun() ->
                                    lib_task:event(use_peach, {GoodsInfo#goods.goods_id}, PlayerStatus) end);
                                false ->
                                    spawn(fun() ->
                                        lib_task:event(use_goods, {GoodsInfo#goods.goods_id}, PlayerStatus) end)
                            end,
            ...
            case (catch lib_make:strengthen(PlayerStatus, GoodsStatus, GoodsInfo, StoneInfo, RuneInfo1, N1, RuneInfo2, N2, RuneInfo3, N3, RuneInfo4, N4, Proinfo, GoodsStrengthenRule, GoodsStrengthenAntiRule, Auto_purch, GoldCost)) of
                {ok, NewPlayerStatus, NewStatus, [Result, NewStrengthen, NewStoneNum, StrenFail, NN1, NN2, NN3, NN4]} ->
                    if Auto_purch == 1 andalso is_record(StoneInfo, goods) == false andalso GoldCost > 0 ->
                        spawn(fun() ->
                            log:log_shop([1, 1, PlayerStatus#player.id, PlayerStatus#player.nickname, StoneTypeId, gold, GoldCost, 1]) end);
                        true ->
                            skip
                    end,
            ...
            case (catch lib_make:compose(PlayerStatus, GoodsStatus, NewStoneList, TotalStoneNum, RuneInfo, GoodsComposeRule, RuneTypeId, Auto_purch, GoldCost)) of
                {ok, NewPlayerStatus, NewStatus, NewGoodsTypeId} ->
                    if Auto_purch == 1 andalso is_record(RuneInfo, goods) == false andalso GoldCost > 0 ->
                        spawn(fun() ->
                            log:log_shop([1, 1, PlayerStatus#player.id, PlayerStatus#player.nickname, RuneTypeId, gold, GoldCost, 1]) end);
                        true ->
                            skip
                    end,
                    ...
                    case Oper == 1 of %%附魔
                        true ->
                            if Auto_purch == 1 andalso is_record(MagicStoneInfo, goods) == false andalso GoldCost > 0 ->
                                spawn(fun() ->
                                    log:log_shop([1, 1, PlayerStatus#player.id, PlayerStatus#player.nickname, MagicStoneTypeId, gold, GoldCost, 1]) end);
                                true ->
                                    skip
                            end,
...
check_pay(PlayerStatus, GoodsStatus, GoodsTypeId, GoodsNum, ShopType, ShopSubtype) ->
    ...
    ?ASSERT(is_record(ShopInfo, ets_shop), {fail, ?E_SHOP_NIU_2}),
    _GoodsTypeInfo = goods_util:get_goods_type(GoodsTypeId),
    ?ASSERT(is_record(_GoodsTypeInfo, ets_base_goods), {fail, ?E_SHOP_NIU_3}),
    ?ASSERT(GoodsNum > 0, {fail, ?E_SHOP_NIU_1}),
    ?IF(ShopType =:= 1 andalso ShopSubtype =:= 7, throw({fail, ?E_SHOP_NIU_6})),
    %%开服特惠商品
    ...
    _GoodsTypeInfo2 =
        ...
        %%特惠区和礼券区、积分商店出售的物品绑定
            ShopType =:= 1 andalso (ShopSubtype =:= 5 orelse ShopSubtype =:= 6 orelse ShopSubtype =:= 7) ->
                _GoodsTypeInfo#ets_base_goods{bind = 2};
        %%某些商店类型绑定
    ...
    GoodsTypeInfo =
        case lists:member(_GoodsTypeInfo2#ets_base_goods.goods_id, [16008]) of
            true ->
                ...
                _GoodsTypeInfo2#ets_base_goods{expire_time = Expire};
            false ->
                _GoodsTypeInfo2
        end,
    %% 检查背包是否足够
    ...
    %% 商城
        ShopType == 1 ->
            ...
            ?ASSERT(goods_util:is_enough_money(PlayerStatus, Cost, PriceType), {fail, ?E_SHOP_NEED_ITEM_NOT_ENOUGH}),
            ?IF(GoodsTypeInfo#ets_base_goods.goods_id == 28118 andalso GoodsNum > 1, throw({fail, ?E_SHOP_NIU_4})),
            ?IF(OnlyOneTime andalso BuyTimes > 0, throw({fail, ?E_SHOP_NIU_5})),
...
check_strengthen(PlayerStatus, GoodsId, StoneId, RuneId1, _N1, RuneId2, _N2, RuneId3, _N3, RuneId4, _N4, Protect, StoneTypeId, Auto_purch) ->
    ...
    case RuneId1 > 0 of
        ...
        false ->
            RuneInfo1 = #goods{num = 0, player_id = PlayerStatus#player.id, id = 0, goods_id = 0, location = 4, type = 15, subtype = 11}
    end,
    case RuneId2 > 0 of
        true -> RuneInfo2 = goods_util:get_goods(RuneId2);
        false ->
            RuneInfo2 = #goods{num = 0, player_id = PlayerStatus#player.id, id = 0, goods_id = 0, location = 4, type = 15, subtype = 11}
    end,
    case RuneId3 > 0 of
        true -> RuneInfo3 = goods_util:get_goods(RuneId3);
        false ->
            RuneInfo3 = #goods{num = 0, player_id = PlayerStatus#player.id, id = 0, goods_id = 0, location = 4, type = 15, subtype = 11}
    end,
    case RuneId4 > 0 of
        true -> RuneInfo4 = goods_util:get_goods(RuneId4);
        false ->
            RuneInfo4 = #goods{num = 0, player_id = PlayerStatus#player.id, id = 0, goods_id = 0, location = 4, type = 15, subtype = 11}
    end,
    case Protect > 0 of
        true -> Proinfo = goods_util:get_goods(Protect);
        false ->
            Proinfo = #goods{num = 0, player_id = PlayerStatus#player.id, id = 0, goods_id = 0, location = 4, type = 15, subtype = 11}
    end,
