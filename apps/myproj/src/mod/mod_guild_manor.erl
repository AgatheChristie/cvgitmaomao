...
get_guild_manor_pid(SceneId, GuildId, PlayerId, PlayerPid) ->
    ...
    case misc:whereis_name({global, ProcessName}) of
        Pid when is_pid(Pid) ->
%% 				?DEBUG("the process is built:~p\n", [Pid]),
            add_manor_role_list(Pid, PlayerId, PlayerPid),
            {ok, Pid};
        _OtherPid ->
%% 				?DEBUG("OMG, the process is no built:~p\n", [_OtherPid]),
...
%% 	?DEBUG("terminate the guild manor! OK \n"),
    ok.
